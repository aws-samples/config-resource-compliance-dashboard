# AWS Config Resource Compliance Dashboard (CRCD) Customer Incident Response Team (CIRT) Conformance Pack
...
The AWS Config Resource Compliance Dashboard (CRCD) Customer Incident Response Team (CIRT) Conformance Pack is a comprehensive compliance monitoring solution that deploys AWS Config rules recommended by the AWS Customer Incident Response Team (CIRT). This conformance pack helps customers maintain security and compliance best practices across their AWS environments.

## Features
- **CIRT-Recommended Rules**: Includes both standard AWS Config managed rules and custom Lambda-backed rules recommended by AWS CIRT
...
- **CRCD-CIRT-Security-Recommendations** containing all standard AWS Config rules deployed in all regions and all accounts of your organizations
- **CRCD-CIRT-Security-Recommendations-IA-IAM** containing all rules that apply to IAM resources, including the custom rules. Since IAM resources are global, these rules can be deployed on one region to avoid redundancy.

The conformance packs support all parameters to their AWS Config rules and manage automatically regional availability of rules - i.e. you can deploy the conformance pack in all Regions where AWS Config is enabled, and if a rule is not available in a region, it will be automatically skipped.
...
1. Follow the link to open the cloud formation [template](https://console.aws.amazon.com/cloudformation/home#/stacks/create/review?&templateURL=https://aws-managed-cost-intelligence-dashboards.s3.amazonaws.com/cfn/crcd-cirt-conformance-pack.yaml&stackName=crcd-cirt-conformance-pack-resources) in your CloudFormation console. 
