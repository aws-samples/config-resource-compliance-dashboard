...
def process_file_backup():    
    try:
        ...
        # Use Glue's native S3 reading capabilities
        try:
            ...
            # If the DataFrame is empty, it is an error
            if dataframe_count == 0:
                logger.info("====CRCD==== 30 ERROR")
                error_message = "Failed to read any data from the source file"
                logger.error(error_message)
                update_job_status('FAILED', 0, error_message)
                raise ValueError(error_message)
        ...
        logger.info(f"====CRCD==== file_version = {file_version}")
        logger.info(f"====CRCD==== snapshot_id = {snapshot_id}")
        ...
        # Add resourceType as a separate column for grouping
        items_with_type_df = items_df.withColumn(
            "resourceType", 
            when(col("item.resourceType").isNotNull(), col("item.resourceType")).otherwise(lit("unknown"))
        )
        
        # Group by resource type - this creates a DataFrame with resourceType and items_array columns
        grouped_df = items_with_type_df.groupBy("resourceType").agg(
            collect_list("item").alias("items_array")
        )
        
        logger.info("====CRCD==== finished grouping with dataframes")

        # Track total items processed
        processed_count = 0

        # Process each resource type group
        # Processing Approach: Instead of using RDD's mapPartitions, 
        # I collect the grouped DataFrame and process each group separately. 
        # For very large datasets, you could consider using foreachPartition on the DataFrame [10].
        # https://docs.aws.amazon.com/glue/latest/dg/transforms-configure-select-collection.html
        def process_resource_group(resource_type, items_array):
            nonlocal processed_count
            local_s3 = boto3.client('s3')
            batch_size = 200  # was 100
            
            # Convert items to dictionaries if they're Row objects
            if hasattr(items_array[0], "asDict"):
                items_list = [item.asDict() for item in items_array]
            else:
        ...
        # Collect the grouped data for processing
        # For large datasets, consider using foreachPartition instead
        resource_groups = grouped_df.collect()
        
        # Process each resource type group
        for row in resource_groups:
            process_resource_group(row["resourceType"], row["items_array"])
        
        logger.info(f"====CRCD==== Processing Summary:")
        logger.info(f"====CRCD==== Total items processed: {processed_count}")
        ...
        # RDD stands for Resilient Distributed Dataset, which is the fundamental 
        # data structure in Apache Spark. An RDD is an immutable, distributed collection 
        ...
        logger.info("====CRCD==== finished grouping")
